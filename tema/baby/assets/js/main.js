...
// Dummy Countdown
	let current = new Date()
		dd = String(current.getDate()).padStart(2,'0')
		mm = String(current.getMonth() + 1).padStart(2,'0')
		yyyy = current.getFullYear()
		cddDD = current.getDate()
		cddMM = current.getMonth() + 1
		cddYY = current.getFullYear()
		cddS = Math.floor((current.getTime() / 1000) % 60)
		cddM = Math.floor((current.getTime() / (1000 * 60)) % 60)
		cddH = (Math.floor((current.getTime() / (1000 * 60 * 60)) % 24)) + 7
		dummyE1CD = new Date(new Date().setDate(new Date().getDate() + 40))
		dummyE2CD = new Date(new Date().setDate(new Date().getDate() + 44))
		dummyDays = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu']
		dummyMonths = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
	var dummyEvent1 = dummyDays[dummyE1CD.getDay()]  + ', ' + dummyE1CD.getDate() + ' ' + dummyMonths[dummyE1CD.getMonth()] + ' ' + dummyE1CD.getFullYear()
	var dummyEvent2 = dummyDays[dummyE2CD.getDay()]  + ', ' + dummyE2CD.getDate() + ' ' + dummyMonths[dummyE2CD.getMonth()] + ' ' + dummyE2CD.getFullYear()
	// if ( window.location.href.indexOf('?') > -1 ) {
	// 	var datePublishStart = new Date(cddYY, cddMM-1, cddDD, cddH, cddM, cddS+0)
	// } else {
	// 	var datePublishStart = new Date(cddYY, cddMM-1, cddDD, cddH, cddM, cddS-10)
	// }
	// var datePublishEnd = new Date(cddYY, cddMM-1, cddDD, cddH, cddM, cddS+10)

// Data
	const datePublishStart = new Date(2022, 12-1, 31, 0, 0, 0)
	const datePublishEnd = new Date(2024, 12-1, 31, 0, 0, 0)

	const md5security = 'tema-dalem security'
	const md5account = 'tema-dalem account'
	// https://www.md5hashgenerator.com/
	// const string = 'https://maumantenan.com/tema/dalem/'
	const md5 = '12227095ecd4210f47dd7e896c63c931'
	// const sha1 = 'ccdf2abe3819fe45bf7b7bf6ecc6b94f149e27cc'

	// let shUrl = 'https://docs.google.com/spreadsheets/d/1eLzBeZf-s2gOu0GEboeSbdwuqCmM4IVl_JzFCK87Bjk/gviz/tq?tqx=out:csv&sheet='
		// gsStart = 'https://script.google.com/macros/s/'
		// gsEnd = '/exec'
		// urlBingkisan = gsStart + 'AKfycbxh8MkgUaVxbcE1rlDgZlfhh-S-xHpYIGN-rRXLyABfdn1wpZN1GbCdrgRU8cA5t0E' + gsEnd
		// urlKehadiran = gsStart + 'AKfycbz7FBoc83rrxXdj-B2L_l1P5x2Ou1Af64lWFHy7D5lueSYCFaupQBN1BbuAaD2IOy77' + gsEnd
		// urlUcapan = gsStart + 'AKfycbzwfP5NyxktvNr3LXtkVCNFXuwO1R97YObHCo6QNyczSHqqSPDEMjBCy3mnky13JbhQ' + gsEnd
	const shUrl = atob('aHR0cHM6Ly9kb2NzLmdvb2dsZS5jb20vc3ByZWFkc2hlZXRzL2QvMWVMekJlWmYtczJnT3UwR0Vib2VTYmR3dXFDbU00SVZsX0p6RkNLODdCamsvZ3Zpei90cT90cXg9b3V0OmNzdiZzaGVldD0=')
	const urlBingkisan = atob('aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J4aDhNa2dVYVZ4YmNFMXJsRGdabGZoaC1TLXhIcFlJR04tclJYTHlBQmZkbjF3cFpOMUdiQ2RyZ1JVOGNBNXQwRS9leGVj')
	const urlKehadiran = atob('aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J6N0ZCb2M4M3JyeFhkai1CMkxfbDFQNXgyT3UxQWY2NGxXRkh5N0Q1bHVlU1lDRmF1cFFCTjFCYnVBYUQySU95NzcvZXhlYw==')
	const urlUcapan = atob('aHR0cHM6Ly9zY3JpcHQuZ29vZ2xlLmNvbS9tYWNyb3Mvcy9BS2Z5Y2J6d2ZQNU55eGt0dk5yM0xYdGtWQ05GWHV3TzFSOTdZT2JIQ282UU55Y3pTSHFxU1BERU1qQkN5M21ua3kxM0piaFEvZXhlYw==')
	...
	$.ajax({
		...
		success: function(data) {
			...
			$(undangan).each(function (i,list) {
				...
				// 	acara3jam = list[6]
				let acara1tanggal = dummyEvent1
					acara1jam = list[2]
					acara2s = dummyEvent1.split(' ')
					acara2 = acara2s[2] + ' ' + acara2s[3]
					acara2tanggal = dummyEvent1
					acara2jam = list[4]
					acara3s = dummyEvent2.split(' ')
					acara3 = acara3s[2] + ' ' + acara3s[3]
					acara3tanggal = dummyEvent2
					acara3jam = list[6]
					acara2sesi1 = list[7]
					acara2sesi2 = list[8]
					acara2sesi3 = list[9]
				...
				const event1place = `Kediaman Mempelai Putri<br>Jl. Melati No. 12 Caturtunggal Depok <br class="d-none d-sm-block">Sleman`
				...
				const event2title = `Tasyakuran`
				const event2intro = `Sleman, ` + acara2
				const event2date = acara2tanggal
				const event2time = `Pukul ` + acara2jam
				const event2place = `Kediaman kami<br>Jl. Sosio Humaniora No. 1 Karang Malang <br>Caturtunggal Depok Sleman`
				const event2map = `https://goo.gl/maps/weiSwGEsa2oCfz3f6`
				...
				const event3intro = `Semarang, ` + acara3
				const event3date = acara3tanggal
				const event3time = `Pukul ` + acara3jam
				const event3place = `Kediaman Mempelai Putra<br>Perum Nusantara Indah No. 46 Tembalang<br> Semarang`
				const event3map = `https://goo.gl/maps/EJ8ZtBdiEZZyQcSx7`
				...
				const person1name = `Gina Putri Amalia`
				const person1nick = `Gina`
				const person1father = `Mustaji Mugiono`
				const person1mother = `Dewi Kurniasih`
				const person1place = `Jl. Melati No. 12 Caturtunggal Depok <br class="d-none d-sm-block">Sleman`
				const person1gift1 = `1370016317XXX`
				const person1gift2 = `<b>` + person1name + `</b><br>Jl. Melati No. 12 Caturtunggal Kec. Depok Kab. Sleman`
				...
				const person2name = `Budi Heriyanto`
				const person2nick = `Budi`
				const person2father = `Slamet Budiman`
				const person2mother = `Eliana Putri`
				const person2place = `Perum Nusantara Indah No. 46 Tembalang <br class="d-none d-sm-block">Semarang`
				const person2gift1 = `1370016317XXX`
				const person2gift2 = `<b>` + person2name + `</b><br>Perum Nusantara Indah No. 46 Kramas Tembalang Semarang`
